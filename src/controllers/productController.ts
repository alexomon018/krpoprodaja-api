...
export async function getProductById(req: Request, res: Response) {
  try {
    ...
    // Get total item count for this seller
    const [{ itemsOnSale }] = await db
      .select({ itemsOnSale: sql<number>`count(*)::int` })
      .from(products)
      .where(
        and(
          eq(products.sellerId, product.seller.id),
          eq(products.status, "active")
        )
      );

    const [{ itemsSold }] = await db
      .select({ itemsSold: sql<number>`count(*)::int` })
      .from(products)
      .where(
        and(
          eq(products.sellerId, product.seller.id),
    ...
    // Add item counts to seller info
    const productWithItemCount = {
      ...processedProduct,
      seller: {
    ...
    return res.status(200).json(productWithItemCount);
  } catch (error) {
