...
import { generateVerificationCode, sendPhoneVerificationSMS } from '../services/snsService.ts'
  ...
  try {
    ...
    if (!phoneRegex.test(phone)) {
      return res.status(400).json({
        error: 'Invalid phone number format. Please use E.164 format (e.g., +381601234567)',
      })
    }
